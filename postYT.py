...
#give values that are not id or name for error testing / give a channel that does not exist i.e incorrect channel name
def get_channel():
...
def create_info_table(response):
  try:
    ...
    channelData_Dataframe = pd.DataFrame.from_dict(channelData, orient='index', columns = ['Channel Data'])
    engine = db.create_engine('sqlite:///data_base_name.db')
    channelData_Dataframe.to_sql('channelData', con=engine, if_exists='replace', index=True)
